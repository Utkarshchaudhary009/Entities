    ...
    const handlePublish = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { error: dbError } = await supabase.from("blogs").insert({
                ...
                author_id: user?.id, // Assuming author_id maps to user_id string for now, or you need an authors table map
                is_published: true,
            });

            // Note: If author_id is a UUID FK to an 'authors' table, we might need a mapping step.
            // For this MVP, assuming simplicity or strict schema adherence.
            // If schema.sql requires 'authors' table entry, we might fail here if not handled.
            // Let's assume for now user.id is valid or we'll catch the error.
